"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TblUserService = void 0;
const common_1 = require("@nestjs/common");
const tbl_user_entity_1 = require("./entities/tbl_user.entity");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
let TblUserService = class TblUserService {
    constructor(TblUserRepository) {
        this.TblUserRepository = TblUserRepository;
    }
    async create(createTblUserDto) {
        const _user = this.TblUserRepository.create(createTblUserDto);
        return await this.TblUserRepository.save(_user);
    }
    async findAll() {
        return await this.TblUserRepository.find({
            relations: {
                _driver: true,
            }
        });
    }
    async findOne(user_id) {
        const _user_id = await this.TblUserRepository.findOneBy({ user_id });
        if (!_user_id) {
            throw new common_1.BadRequestException('User not found');
        }
        return _user_id;
    }
    async update(user_id, updateTblUserDto) {
        await this.findOne(user_id);
        return await this.TblUserRepository.update(user_id, updateTblUserDto);
    }
    async remove(user_id) {
        await this.findOne(user_id);
        return await this.TblUserRepository.delete({ user_id });
    }
};
exports.TblUserService = TblUserService;
exports.TblUserService = TblUserService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(tbl_user_entity_1.TblUser)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], TblUserService);
//# sourceMappingURL=tbl_user.service.js.map